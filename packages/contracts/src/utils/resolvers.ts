...
import { resolveName } from "./namedAddress";
...
export const resolveAddress = (
  ...
  if (!addressContractNameSymbol.match(ethereumAddress)) {
    ...
    // If a named contract
    address = resolveName(addressContractNameSymbol as ContractNames, chain);

    if (!address) {
      // If a token Symbol
      const token = tokens.find(
        (t) => t.symbol === addressContractNameSymbol && t.chain === chain
      );
      if (!token)
        throw Error(
          `Invalid address, token symbol or contract name "${addressContractNameSymbol}" for chain ${chain}`
        );
      if (!token.address)
        throw Error(
          `Can not find address for token "${addressContractNameSymbol}" on chain ${chain}`
        );

      address = token.address;
      log(
        `Resolved asset with symbol "${addressContractNameSymbol}" to address ${address}`
      );
    ...
    log(
      `Resolved contract name "${addressContractNameSymbol}" to address ${address}`
    );
...
export const resolveToken = (symbol: string, chain = Chain.mainnet): Token => {
  ...
  if (!token.address)
    throw Error(`Can not find token for ${symbol} on chain ${chain}`);
...
export const resolveVaultToken = async (
  ...
  if (isAddress(addressContractNameSymbol)) {
    const tkn = IERC20Metadata__factory.connect(
      addressContractNameSymbol,
      signer
    );
    token = {
