...
subtask('token-transfer', 'Transfer an amount of tokens from the signer to the recipient')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const desc = `${signerAddress} transfers ${formatUnits(amountBN, tokenConfig.decimals)} ${tokenConfig.symbol} to ${recipientAddress}`;
...
subtask('token-allowance', 'Logs the amount of tokens a spender can transfer from an owner')
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    log(`Spender ${spenderAddress} can transfer ${formatUnits(amount, tokenConfig.decimals)} ${tokenConfig.symbol} from ${ownerAddress}`);
  });
